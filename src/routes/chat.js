...
router.post('/message', validateBody(schemas.sendMessage), asyncHandler(async (req, res) => {
  ...
  if (existingIdeas?.generated) {
    ...
    if (selectedNumber) {
      ...
      return res.json({ 
        success: true, response, ideaSelected, 
        phase: 'validation', phaseChanged: true,
        progress, agent: 'IdeaGenerator' 
      });
    }
  }
  ...
  if (isBackToIdeasRequest(lowerMessage)) {
    ...
    const progress = await orchestrator.getProgress(sessionId);
    return res.json({ 
      success: true, response, backToIdeas, 
      phase: 'ideation', phaseChanged: true,
      progress, agent: 'IdeaGenerator' 
    });
  }
...
function parseIdeaSelection(message) {
  ...
  if (ideaMatch) return parseInt(ideaMatch[1]);
  
  for (const [word, num] of Object.entries(wordSelections)) {
    if (lowerMessage.includes(word)) return num;
  }
 ...
 */
router.get('/progress/:sessionId', asyncHandler(async (req, res) => {
  const { sessionId } = req.params;
